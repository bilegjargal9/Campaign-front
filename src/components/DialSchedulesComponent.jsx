//DialSchedulesComponent.jsx
import React, { useState, useEffect } from 'react';
import { Calendar, Check, X, Phone, AlertCircle, ChevronDown, ChevronRight, RefreshCw } from 'lucide-react';
import dialApi from '../api/dial';
import '../styles/customer.css';


const DialSchedulesComponent = ({ notifierRef }) => {
  ...
  const loadSchedules = async () => {
    try {
      setLoading(true);
      const data = await dialApi.getAllSchedule();
      setScheduleData({
  ...
  const findSchedulesGroup = async () => {
    try {
      ...
      for (const groupId of uniqueGroupIds) {
        try {
          const response = await dialApi.findGroup(groupId);
  ...
  const handleDeleteSelectedSchedule = async () => {
    ...
    try {
      ...
      for (const scheduleId of selectedSchedules) {
        await dialApi.deleteSchedule(scheduleId);
      }
      
      notifierRef.current.show(`${selectedSchedules.length} scheduled calls deleted`, 'success');
      loadSchedules();
      setSelectedSchedules([]);
    } catch (error) {
      notifierRef.current.show(
  ...
  const handleRetryFailedCall = async (scheduleId) => {
    try {
      setLoading(true);
      await dialApi.retryCall(scheduleId);
      notifierRef.current.show('Call retry requested successfully', 'success');
      loadSchedules();
    } catch (error) {
      notifierRef.current.show(
        error.response?.data?.message || 'Failed to retry call',
        'error'
      );
    } finally {
      setLoading(false);
      ...
      return (
        <div className="flex gap-2 items-center">
          ...
          <button 
            onClick={(e) => {
              e.stopPropagation();
              handleRetryFailedCall(schedule.id_uuid);
            }} 
    ...
    <div className="bg-white p-6 rounded-lg shadow-md mb-8">
      ...
      {loading && !Object.keys(groupedSchedules).length ? (
        ...
        <div className="text-center py-6 bg-gray-50 border border-gray-100 rounded-lg">
          ...
          <p className="text-gray-500">Дуудлагын хуваарь байхгүй байна.</p>
        </div>
      ) : (
        <>
          ...
          {Object.entries(groupedSchedules).map(([date, timeGroups]) => (
            <div key={date} className="mb-6">
              <h3 className="text-md font-medium mb-2 flex items-center gap-2 bg-gray-50 p-2 rounded">
                <Calendar size={16} />
                {date} ({Object.values(timeGroups).flat().reduce((acc, group) => acc + group.schedules.length, 0)} дуудлага)
              </h3>
                ...
                <table className="min-w-full bg-white border border-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="py-2 px-3 text-left" width="160">Цаг</th>
                      <th className="py-2 px-3 text-left " width="300">Бүлэг</th>
                      <th className="py-2 px-3 text-left">Хүлээн авагчид</th>
                      <th className="py-2 px-3 text-left" width="250">Мессеж</th>
                          ...
                          <React.Fragment key={groupId}>
                            ...
                            >
                              ...
                              <td className="py-2 px-3 flex items-center gap-1">
                                ...
                                <span className="ml-2 text-xs text-gray-500">
                                  ({group.schedules.length} дуудлага)
                                </span>
                              </td>
                              <td className="py-2 px-3">
                                {group.schedules.length > 1 ? 
                                  `${group.schedules.length} recipients` : 
                                  group.schedules[0]?.customer?.phone_number || 'Unknown recipient'
                                }
                              </td>
                              <td className="py-2 px-3">
                                {group.schedules[0]?.template?.message ? 
                                  (group.schedules[0].template.message.length > 30 ? 
                                    `${group.schedules[0].template.message.substring(0, 30)}...` : 
                                    group.schedules[0].template.message) : 
                                  'No message'
                                }
                                {(group.schedules[0]?.template?.audio_url) && 
                                  <span className="text-xs text-blue-500 ml-2">[Audio]</span>
                                }
                                {group.schedules.length > 1 && group.schedules.some(
                                  s => s.template?.message !== group.schedules[0]?.template?.message
                                ) && ' (multiple)'}
                              </td>
                              <td className="py-2 px-3">
                            ...
                            {isExpanded && group.schedules.map(schedule => (
                              <tr key={schedule.id_uuid} className="bg-gray-100">
                                ...
                                <td className="py-2 px-3">
                                  {schedule.customer?.phone_number+" : "+schedule.customer?.first_name || 'Unknown recipient'}
                                </td>
                                <td className="py-2 px-3">
                                  {schedule.template?.message ? 
                                    (schedule.template.message.length > 30 ? 
                                      `${schedule.template.message.substring(0, 30)}...` : 
                                      schedule.template.message) : 
                                    'No message'
                                  }
                                  {(schedule.template?.audio_url) && 
                                    <span className="text-xs text-blue-500 ml-2">[Audio]</span>
                                  }
                                </td>
                                <td className="py-2 px-3">
...
export default DialSchedulesComponent;
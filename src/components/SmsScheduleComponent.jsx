//SmsSchedulesComponent.jsx
import React, { useState, useEffect } from 'react';
import { Calendar, Check, X, MessageSquare, AlertCircle, ChevronDown, ChevronRight, Receipt } from 'lucide-react';
import smsApi from '../api/sms';
import '../styles/customer.css'

const SmsSchedulesComponent = ({ notifierRef }) => {
  ...
  const loadSchedules = async () => {
    try {
      setLoading(true);
      const data = await smsApi.getSchedule();
      setScheduleData({
  ...
  const findSchedulesGroup = async () => {
    try {
      ...
      for (const groupId of uniqueGroupIds) {
        try {
          const response = await smsApi.findGroup(groupId);
  ...
  const handleDeleteSelectedSchedule = async () => {
    ...
    try {
      ...
      for (const scheduleId of selectedSchedules) {
        await smsApi.deleteSchedule(scheduleId);
      }
      
      notifierRef.current.show(`${selectedSchedules.length} scheduled SMS deleted`, 'success');
      loadSchedules();
      setSelectedSchedules([]);
    } catch (error) {
      notifierRef.current.show(
    ...
    <div className="bg-white p-6 rounded-lg shadow-md mb-8">
      ...
      {loading ? (
        ...
        <div className="text-center py-6 bg-gray-50 border border-gray-100 rounded-lg">
          ...
          <p className="text-gray-500">No scheduled SMS found</p>
        </div>
      ) : (
        <>
          ...
          {Object.entries(groupedSchedules).map(([date, timeGroups]) => (
            <div key={date} className="mb-6">
              <h3 className="text-md font-medium mb-2 flex items-center gap-2 bg-gray-50 p-2 rounded">
                <Calendar size={16} />
                {date} ({Object.values(timeGroups).flat().reduce((acc, group) => acc + group.schedules.length, 0)} зурвасууд)
              </h3>
                            ...
                            >
                              ...
                              <td className="py-2 px-3 flex items-center">
                                ...
                                <span className="ml-2 text-xs text-gray-500">
                                  ({group.schedules.length} зурвасууд)
                                </span>
                              </td>
                              <td className="py-2 px-3">
                                {group.schedules.length > 1 ? 
                                  `${group.schedules.length} recipients` : 
                                  group.schedules[0]?.customer?.first_name +" : "+group.schedules[0]?.customer?.phone_number || 'Unknown recipient'
                                }
                              </td>
                              <td className="py-2 px-3">
...
export default SmsSchedulesComponent;